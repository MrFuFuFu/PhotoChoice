...
public class FloderAlbumGridAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(mIsVideo){
        	...
        	Bitmap bitmap = NativeImageLoader.getInstance().loadNativeImage(true, entry.bucketUrl, mPoint, new NativeImageLoader.NativeImageCallBack() {
				@Override
			...
			holder.iv_folder_icon_play.setVisibility(View.GONE);
			Picasso.with(mContext).load(new File(entry.bucketUrl))
			.resize(mWidth/2, mWidth/2).centerCrop().placeholder(R.mipmap.default_videoimg).into(holder.imageView);
		}
		return convertView;
	}
